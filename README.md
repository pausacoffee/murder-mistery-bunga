...
- 타이머
  ...
  - 플레이어별 탭 타이머
- 규칙 팝업
  - 책 아이콘: 게임 전체 규칙 탭형 팝업
  - 페이지별 `i` 버튼: 현재 단계의 단일 규칙 팝업
- 게임별 동적 헤더 액션
...
## 규칙 팝업 연결 규칙
- 책 아이콘 버튼은 `game.json > rules.tabs` 전체를 여는 버튼입니다.
- `i` 버튼은 현재 슬라이드의 단계 규칙만 여는 숏컷입니다.
...
- `i` 버튼이 단순히 큰 분류를 열기보다, 지금 단계에서 바로 확인해야 하는 규칙 1개를 열도록 설계하는 것을 권장합니다.
